...
## Consignes
Vous devez écrire une classe `Chat` qui possède des attributs, des méthodes et un constructeur par défaut. Vous êtes libre de conceptualiser ce qu'est un chat en utilisant votre créativité. Par contre, il faut que votre classe contiennent **au moins** :

* 2 attributs;
* 2 méthodes;
* 1 constructeur.

Pour évaluer le fonctionnement de votre classe personnalisée, créez un chat et exécuter une série de tests. Dans vos tests, souligner le problème de validation vu dans la [capsule #2](../classeInv/capsule2.html) en mettant un chat dans un état qui serait considéré invalide.
...
### Conceptualisation
Identifiez des attributs que tous les chats possèdent. Par exemple, tous les chats ont un nombre de patte, une couleur, etc.
