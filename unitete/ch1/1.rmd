...
Comme la majorité des moteurs de jeux, celui d'Unity est écrit en C++. Par contre, le langage utilisé pour communiquer avec le moteur est C#. Le moteur possède un API[^1] qui permet à Mono (une machine virtuelle pour C#) de communiquer avec. L'API rend accessibles des fonctions C# avec une bibliothèque (notamment UnityEngine) qui, une fois appelée par Mono, exécute une fonction C++ du moteur. C'est ainsi que C# est nommé un langage de _scripting_, dans le contexte d'Unity.

Dans ce modèle, le moteur est un serveur et le code C# est le client.	

**N.B.** Il est aussi possible d'utiliser la machine virtuelle LLVM en compilant le C# en code natif avec IL2CPP.
