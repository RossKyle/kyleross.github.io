...
* `Déplacer(...)` : cette méthode doit prendre en paramètre un `Vector2i` qui représente le _delta_ de position à faire. Par exemple, le delta pourrait être de (1,0), ce qui déplacera l'entité à droite d'un unité. En plus de tenter de changer la position de l'entité, cette méthode doit retourner un `bool` indiquant si le déplacement est réussit ou non. Il est possible que déplacer une entité soit impossible si :
...
* `MettreÀJour()` : cette méthode sera appelée à chaque mise à jour de la scène (l'univers du jeu). C'est ici que la logique principale des entités se fait. Par exemple, pour le joueur, l'état des touches pressées est vérifié et le joueur se déplace en accord. Pour un ennemi, il est déplacer dans sa direction de patrouille. Cette méthode doit être `public`.
* `SubirDégats` : cette méthode doit prendre en paramètre un `int` qui représente des dégâts à infliger sur l'entité. Par défaut, un entité perd un nombre de point de vie équivalent aux dégâts à infliger moins sa défense. Par contre, un entité doit toujours subir au moins un dégat. Cette méthode doit être `public`.
...
* un comportement de patrouillage — les ennemis se déplace dans une direction choisie au hazard tant qu'ils ne peuvent continuer. S'il est impossible de se déplacer, leur direction de patrouillage est inversée.
* Un constructeur paramétrique qui prend en paramètre une `Texture` et un `Vector2i`. La texture est celle qui sera appliqué au `Sprite` de l'ennemi et le vecteur représente la position initiale de l'ennemi.
...
* une méthode `MettreÀJour()` qui déplace l'ennemi à chaque 60 appel de mise à jour (à chaque 60 frames). Créez un compteur pour cette tâche;
* 10 PV et 2 DEF.
...
**N.B. : ** Ceci ne sera pas tester _in game_, car le joueur ne peut pas attaquer les ennemis dans cette version du jeu. Vous devez tout de même implanter ce comportement.
